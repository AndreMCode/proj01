...
public class Wandering2AI : MonoBehaviour
{
    ...
    private readonly float sphereCastRadius = 0.5f; // was 2.5
        ...
        if (isAlive && !playerDetected)
        {
            ...
            if (Physics.SphereCast(ray, sphereCastRadius * 4.0f, out RaycastHit hit, 100f, layerMask))
            {
                ...
                if (hitObject.GetComponent<PlayerCharacter>() && !fireballCooldown)
                {
                    ...
                    fireball = Instantiate(largeFireballPrefab) as GameObject;
    ...
    private IEnumerator FireballCooldown()
    {
        ...
        yield return new WaitForSeconds(0.2f); // was 0.5
        fireballCooldown = false;
    }
